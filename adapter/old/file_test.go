...
func TestCourseFiles(t *testing.T) {
	actual, status := ada.CourseFiles("127756", nil)
	...
	AssertDeepEqual(t, actual[0], tab1file1)
	AssertDeepEqual(t, actual[16], tab2file1)
}

func BenchmarkCourseFiles(b *testing.B) {
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		info, status := ada.CourseFiles("127756", nil)
		_ = info
		_ = status
	}
}
